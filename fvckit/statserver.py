...
from fvckit.fvckit_wrappers import process_parallel_lists, deprecated, check_path_existance
import fvckit.frontend
...
__license__ = "LGPL"
__author__ = "Anthony Larcher, Sylvain Meignier, Florent Desnous, Andreas Nautsch (SIDEKIT), Ewald Enzinger (FVCKIT)"
...
def load_existing_statistics_hdf5(statserver, statserver_file_name):
    ...
    :param statserver: fvckit.StatServer to fill
...
+        :return dim_stat1: dimensions of stat1
        ...
        for idx, ss in enumerate(arg):
            assert(isinstance(ss, fvckit.StatServer) and ss.validate()), "Arguments must be proper StatServers"
    ...
    def sum_stat_per_model(self):
        ...
        """
        sts_per_model = fvckit.StatServer()
    ...
    def estimate_between_class(self,
        ...
        for it in range(itNb):
            ...
            if save_partial:
                fvckit.fvckit_io.write_fa_hdf5((mean, V, None, None, sigma),
    ...
    def estimate_within_class(self,
        ...
        for it in range(it_nb):
            ...
            if save_partial:
                fvckit.fvckit_io.write_fa_hdf5((None, None, U, None, None), save_partial + "_{}_within_class.h5")
    ...
    def estimate_map(self, itNb, D, mean, Sigma, Vy=None, Ux=None, num_thread=1, save_partial=False):
        ...
        for it in range(itNb):
            ...
            if save_partial:
                fvckit.fvckit_io.write_fa_hdf5((None, None, None, D, None), save_partial + "_{}_map.h5")
    ...
    def read_subset(statserver_filename, index, prefix=''):
        ...
        with h5py.File(statserver_filename, 'r') as h5f:
            ...
            if isinstance(index, fvckit.IdMap):
                # create tuples of (model,seg) for both HDF5 and IdMap for quick comparaison
                sst = [(mod, seg) for mod, seg in zip(h5f[prefix+"modelset"].value.astype('U', copy=False),
                                                      h5f[prefix+"segset"].value.astype('U', copy=False))]
