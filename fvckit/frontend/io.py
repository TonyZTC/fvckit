...
def read_audio(input_file_name, framerate=None):
    ...
    if framerate is None:
        raise TypeError("Expected sampling frequency required in fvckit.frontend.io.read_audio")
...
def read_label(input_file_name, selected_label='speech', frame_per_second=100, num_frames=None):
    ...
    if len(segments) == 0:
        ...
        if num_frames is None:
            ...
            lbl = numpy.zeros(int(num_frames)).astype(bool)
        ...
        for s in range(len(segments)):
            ...
            if label == selected_label:
                ...
                lbl[int(begin[s]):int(end[s])] = True
    return lbl
